...
Teensy2BridgeNode::Teensy2BridgeNode() : Node("teensy2_bridge_node") {
  ...
  // Individual Range topics for Nav2 obstacle layer
  for (int i = 0; i < 4; i++)
    us_range_pub_[i] = create_publisher<sensor_msgs::msg::Range>(US_TOPICS[i], 10);

  // ── Subscriptions ──────────────────────────────────────────────────────
  // Commands: TURN_CRANK, PRESS_KEYPAD, READ_LED <1-4>, TRANSMIT_IR
...
void Teensy2BridgeNode::handleIncomingLine(const std::string & line) {
  ...
  if (line.rfind("US:", 0) == 0) {
    ...
    while (std::getline(ss, token, ',')) {
      try { distances.push_back(std::stof(token) / 1000.0f); }
      catch (...) { distances.push_back(9.999f); }
    }
    if (distances.size() == 4) publishUltrasonics(distances);
    return;
  }
...
void Teensy2BridgeNode::publishUltrasonics(const std::vector<float> & distances_m) {
  // Publish array for task_manager safety checks
  auto arr = std_msgs::msg::Float32MultiArray();
  arr.data = distances_m;
  us_pub_->publish(arr);

  // Publish individual Range messages for Nav2 obstacle layer
  // HC-SR04 specs: range 2cm-400cm, FOV ~15 degrees
  auto stamp = now();
  for (int i = 0; i < 4; i++) {
    sensor_msgs::msg::Range range;
    range.header.stamp    = stamp;
    range.header.frame_id = US_FRAMES[i];
    range.radiation_type  = sensor_msgs::msg::Range::ULTRASOUND;
    range.field_of_view   = 0.2618f;  // 15 degrees in radians
    range.min_range       = 0.02f;    // 2cm
    range.max_range       = 4.00f;    // 400cm
    range.range           = distances_m[i];
    us_range_pub_[i]->publish(range);
  }
}
