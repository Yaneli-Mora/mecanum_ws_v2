...
TaskManagerNode::TaskManagerNode() : Node("task_manager_node") {
  ...
  //    9. TRANSMIT_IR (send all recorded antenna colors)
  // ══════════════════════════════════════════════════════════════════════
  ...
  mission_queue_ = {
    ...
    // 9. TRANSMIT_IR — send all recorded antenna colors on way home
    { IR },
  };
  ...
  if (phase_ == MissionPhase::RUNNING) {
    ...
    switch (item.type) {
      ...
      case MissionItemType::TRANSMIT_IR:
        // IR transmit — nav to transmit position, raise plank B to 120°, send all colors
