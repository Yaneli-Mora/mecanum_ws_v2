...
// ── Teensy → Pi (sent every control cycle at 100Hz) ──────────────────────
struct __attribute__((packed)) TeensyToPiPacket {
  ...
  uint16_t ultrasonic_mm[4];      // HC-SR04 distances in mm
  uint16_t tof_mm[2];             // VL53L1X left(0) right(1) in mm
  int16_t  flow_vx;               // PMW3901 optical flow x (unused — kept for future)
...
// ── Pi → Teensy (sent every control cycle) ───────────────────────────────
struct __attribute__((packed)) PiToTeensyPacket {
  uint8_t  start_byte       = 0xFE;
  int16_t  motor_cmd[4];          // wheel velocity commands scaled to int16
  uint8_t  checksum;
};

// ── Color zone enum (shared) ──────────────────────────────────────────────
